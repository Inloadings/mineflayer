...
function inject(bot, {physicsEnabled, maxCatchupTicks}) {
    ...
    bot._client.on('explosion', explosion => {
        ...
        if (bot.physicsEnabled && bot.game.gameMode !== 'creative') {
            bot.entity.velocity.x += Math.fround(explosion.playerMotionX)
            bot.entity.velocity.y += Math.fround(explosion.playerMotionY)
            bot.entity.velocity.z += Math.fround(explosion.playerMotionZ)
    ...
    bot.lookAt = async (point, force) => {
        ...
        const yaw = Math.fround(Math.atan2(-delta.x, -delta.z))
        const groundDistance = Math.sqrt(delta.x * delta.x + delta.z * delta.z)
        const pitch = Math.fround(Math.atan2(delta.y, groundDistance))
