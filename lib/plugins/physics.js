...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  function updatePosition(now) {
    ...
    // Apply control state changes only once per tick
    if (pendingSprintState !== null) {
        bot._client.write('entity_action', {
            entityId: bot.entity.id,
            actionId: pendingSprintState ? 3 : 4,
            jumpBoost: 0,
        });
        lastSent.sprintState = pendingSprintState;
        pendingSprintState = null;
    }

    if (pendingSneakState !== null) {
        bot._client.write('entity_action', {
            entityId: bot.entity.id,
            actionId: pendingSneakState ? 0 : 1,
            jumpBoost: 0,
        });
        lastSent.sneakState = pendingSneakState;
        pendingSneakState = null;
    }

    const positionUpdated = lastSent.x !== position.x || lastSent.y !== position.y || lastSent.z !== position.z || ticksSinceLastPosition >= 20;
    ...
    if (positionUpdated) ticksSinceLastPosition = 0;
...
bot.setControlState = (control, state) => {
    ...
    if (control === 'sprint') {
        pendingSprintState = state; // Defer sprint state update
    }
    if (control === 'sneak') {
        pendingSneakState = state; // Defer sneak state update
    }
};

// Initialize pending state variables
let pendingSprintState = null;
let pendingSneakState = null;
