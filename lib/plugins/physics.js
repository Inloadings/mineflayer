...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  function updatePosition(now) {
    ...
    // Defer sending sprint and sneak state updates until the end of the tick
    let sprintChanged = lastSent.sprintState !== controlState.sprint;
    let sneakChanged = lastSent.sneakState !== controlState.sneak;

    if (sprintChanged || sneakChanged) {
        if (sprintChanged) {
    ...
    // Determine the correct packet to send
    ...
    lastSent.onGround = bot.entity.onGround; // Ensure onGround is always updated
}
